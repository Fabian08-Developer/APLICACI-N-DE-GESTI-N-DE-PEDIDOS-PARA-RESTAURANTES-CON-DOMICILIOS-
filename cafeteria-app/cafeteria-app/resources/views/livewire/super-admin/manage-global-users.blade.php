<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-black text-white uppercase tracking-tighter">Usuarios Globales</h2>
            <p class="text-sm text-stone-500 font-medium">Administración de todos los usuarios registrados en el ecosistema</p>
        </div>
    </div>

    {{-- Filtros --}}
    <div class="bg-[#141210] border border-[#292524] rounded-2xl p-4 flex flex-col md:flex-row gap-4">
        ...
        <div class="w-full md:w-48">
            <select wire:model.live="roleFilter" class="w-full bg-[#1C1917] border border-[#44403C] rounded-xl px-4 py-3 text-white text-sm focus:border-[#A85507] focus:ring-0 outline-none">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-[#1C1917]/50 border-b border-[#292524]">
                    <th class="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-stone-500">Usuario</th>
                    <th class="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-stone-500">Empresa</th>
                    <th class="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-stone-500">Rol</th>
            ...
            <tbody class="divide-y divide-[#292524]">
                @forelse($users as $user)
                    <tr class="hover:bg-[#1C1917]/30 transition-colors" wire:key="{{ $user->id }}">
                        ...
                        <td class="px-6 py-4">
                            <span class="px-2 py-0.5 rounded bg-stone-800 text-stone-400 text-[9px] font-black uppercase tracking-widest">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <button wire:click="toggleStatus('{{ $user->id }}')" 
                                        class="p-2 rounded-xl {{ $user->activo ? 'text-amber-500 bg-amber-500/10 hover:bg-amber-500 hover:text-white' : 'text-green-500 bg-green-500/10 hover:bg-green-500 hover:text-white' }} transition-all"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-stone-600 uppercase text-[10px] font-black tracking-widest">No se encontraron usuarios</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if($confirmingDelete)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div class="bg-[#1C1917] border border-rose-500/30 p-8 rounded-[28px] max-w-md w-full shadow-2xl shadow-rose-900/20">
                ...
                <h3 class="text-xl font-black text-white mb-1 tracking-tight">¿Eliminar usuario?</h3>
