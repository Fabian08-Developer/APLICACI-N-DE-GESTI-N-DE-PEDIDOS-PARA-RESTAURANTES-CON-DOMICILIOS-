...
trait OrderEmailNotifications
{
    ...
    {
        if (empty($pago->nequi_correo)) return;

        try {
            match ($estado) {
                EstadoPago::COMPLETADO->value => Mail::to($pago->nequi_correo)->send(new PagoAprobadoMail($pago)),
                EstadoPago::FALLIDO->value    => Mail::to($pago->nequi_correo)->send(new PagoFallidoMail($pago)),
                default                       => null,
            };
        } catch (\Throwable $e) {
    ...
    protected function enviarEmailCancelacion(Pedido $pedido): void
    {
        ...
        $email = $pedido->pagos()->whereNotNull('nequi_correo')->value('nequi_correo');
    ...
    protected function enviarEmailReembolso(Pedido $pedido): void
    {
        $pago = $pedido->pagos()
            ...
            ->whereNotNull('nequi_correo')
            ->first();

        if (!$pago) return;

        try {
            Mail::to($pago->nequi_correo)->send(new ReembolsoMail($pedido, $pago));
        } catch (\Throwable $e) {
