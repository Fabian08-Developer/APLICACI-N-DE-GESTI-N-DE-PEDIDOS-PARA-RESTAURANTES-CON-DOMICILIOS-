        ...
        <table class="table-z">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <span class="badge {{ $zona->activo ? 'badge-success' : 'badge-error' }}">
                            {{ $zona->activo ? 'Activa' : 'Inactiva' }}
                        </span>
                    </td>
                    <td>
                        <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
                            <button class="btn-z btn-z-ghost" onclick="openEditForm({{ json_encode($zona) }}, '{{ $zona->barrios->pluck('nombre')->implode(', ') }}')" style="padding: 0.5rem;">
...
<div class="zonas-grid">
    ...
    <div class="zona-card-premium">
        <div class="zona-card-header">
            ...
            <span class="zona-status-badge {{ $zona->activo ? 'status-activa' : 'status-inactiva' }}">
                {{ $zona->activo ? 'Activa' : 'Inactiva' }}
            </span>
        </div>
        ...
        <div class="zona-card-team">
            ...
            <span class="team-label">
                Domiciliarios asignados ({{ $zona->domiciliarios_count }})
            </span>
        </div>

        <button class="btn-ver-detalle" onclick="viewZoneDetail({{ $zona->id }})">
            Ver detalle
        </button>
    </div>
    @endforeach
...
<script>
    function openCreateForm() {
        ...
        document.getElementById('mainForm').action = "{{ route('admin.zonas.store') }}";
