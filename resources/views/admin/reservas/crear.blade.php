            ...
            <form method="POST" action="{{ route('admin.reservas.store') }}" id="formReservaAdmin">
                ...
                <div x-show="step === 3" x-transition x-cloak>
                    ...
                    <label class="checkbox-label">
                        ...
                        <div>
                            <strong>El cliente pagó el depósito en efectivo o por otro medio de inmediato (<span x-text="mesasSeleccionadas.length > 1 ? '$' + new Intl.NumberFormat('es-CO').format({{ $montoDeposito }} * mesasSeleccionadas.length) + ' (' + mesasSeleccionadas.length + ' mesas × $' + new Intl.NumberFormat('es-CO').format({{ $montoDeposito }}) + ')' : '${{ number_format($montoDeposito, 0, ',', '.') }} (por mesa)'"></span>).</strong><br>
