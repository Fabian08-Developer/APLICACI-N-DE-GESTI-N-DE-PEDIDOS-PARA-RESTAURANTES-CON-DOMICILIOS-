...
<div x-data="salesReport()">
    ...
    <div class="shadcn-filters-container" style="margin-bottom: 2rem;">
        <div class="shadcn-filters">
            <div class="shadcn-select-wrapper">
                <select name="period" x-model="datePeriod" class="shadcn-select">
        ...
        <div x-show="showFilters" x-collapse style="display: none; margin-top: 1rem; position: relative; z-index: 20;">
            <div class="shadcn-card">
                        ...
                        <div class="shadcn-checkbox-group">
                            @foreach($categoriasFiltro as $cat)
                                <label class="shadcn-checkbox-item">
                                    <input type="checkbox" value="{{ $cat->id }}" class="shadcn-checkbox" wire:model.live="categorias">
                    ...
                    <div>
                        ...
                        <div class="shadcn-checkbox-group">
                            @foreach($metodosPagoFiltro as $metodo)
                                <label class="shadcn-checkbox-item">
                                    <input type="checkbox" value="{{ $metodo }}" class="shadcn-checkbox" wire:model.live="metodos_pago">
                    ...
                    <div>
                        ...
                        <div class="shadcn-checkbox-group">
                            @foreach($productosTopFiltro as $prod)
                                <label class="shadcn-checkbox-item">
                                    <input type="checkbox" value="{{ $prod }}" class="shadcn-checkbox" wire:model.live="productos_top">
                ...
                <div style="margin-top: 1.5rem; display: flex; justify-content: flex-end; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 1rem;">
                    <button type="button" class="shadcn-btn" @click="showFilters = false" style="background: var(--primary); color: #0F172A; border-color: var(--primary); height: auto; font-weight: 600;">Cerrar Filtros</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="tarjeta" style="margin-bottom: 2rem;">
        ...
        <div id="chart-tendencia" wire:ignore style="min-height: 280px;"></div>
    </div>
    ...
    <div x-show="showExportSidebar"
        ...
        <div class="exp-panel" id="expPanel">
            ...
            <div class="exp-body" id="expBody">
                ...
                {{-- Tab: Quick Export --}}
                <div class="exp-tab-content" :class="{ 'active': exportTab === 'quick' }" id="exp-tab-quick">
                    <div class="exp-space">
                        {{-- Format --}}
                        <div>
                            <label class="exp-label">Formato de exportación</label>
                            <div class="exp-format-grid">
                                <label class="exp-format-opt" :class="{ 'selected': exportFormat === 'pdf' }" @click="exportFormat = 'pdf'">
                                    <input type="radio" name="exp_format" value="pdf" x-model="exportFormat">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                    <span class="exp-format-label">PDF</span>
                                </label>
                                <label class="exp-format-opt" :class="{ 'selected': exportFormat === 'excel' }" @click="exportFormat = 'excel'">
                                    <input type="radio" name="exp_format" value="excel" x-model="exportFormat">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2"/></svg>
                                    <span class="exp-format-label">EXCEL</span>
                                </label>
                                <label class="exp-format-opt" :class="{ 'selected': exportFormat === 'csv' }" @click="exportFormat = 'csv'">
                                    <input type="radio" name="exp_format" value="csv" x-model="exportFormat">
            ...
            <div class="exp-footer">
                ...
                <button type="button" class="exp-btn exp-btn-primary" :disabled="isExporting" style="min-width:120px;" @click="doExport()">
                    ...
                    <span x-text="isExporting ? 'Exportando...' : 'Exportar'">Exportar</span>
                </button>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.addEventListener('alpine:init', () => {
        ...
        Alpine.data('salesReport', () => ({
            ...
            datePeriod: @entangle('period').live,
            start: @entangle('start').live,
            end: @entangle('end').live,
            categorias: @entangle('categorias').live,
            metodos_pago: @entangle('metodos_pago').live,
            ...
            init() {
                ...
                this.$watch('start', (val) => {
                    if (this.fpInstance && val) {
                        this.fpInstance.setDate([val, this.end], false);
                    }
                });
                this.$watch('end', (val) => {
                    if (this.fpInstance && val) {
                        this.fpInstance.setDate([this.start, val], false);
                    }
                });

                // Initialize charts on nextTick when elements exist in the DOM
                this.$nextTick(() => {
                    this.initCharts();
                });
            ...
            doExport() {
                ...
                if (this.categorias && this.categorias.length) {
                    this.categorias.forEach(cat => params.append('categorias[]', cat));
                }
                if (this.metodos_pago && this.metodos_pago.length) {
                    this.metodos_pago.forEach(mp => params.append('metodos_pago[]', mp));
                }
                if (this.productos_top && this.productos_top.length) {
                    this.productos_top.forEach(pt => params.append('productos_top[]', pt));
                }

                // Preserve token if present
                const urlParams = new URLSearchParams(window.location.search);
                const st = urlParams.get('_st');
                if (st) {
