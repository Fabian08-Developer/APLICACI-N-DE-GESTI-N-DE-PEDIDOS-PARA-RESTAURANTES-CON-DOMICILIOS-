...
<body>
    <div class="contenedor">
        ...
        <div id="pantalla-pendiente" class="tarjeta">
            <div class="pantalla">
                ...
                <p><strong>{{ $pago->telefono }}</strong></p>
                <p style="margin-top: .75rem">
                    Acepta el cobro de <strong>${{ number_format($pedido->total, 2) }}</strong> desde tu app.
                </p>
                <div class="spinner" id="spinner" aria-hidden="true"></div>
                <p id="texto-estado" style="font-size: .83rem">Esperando confirmación…</p>
                <span class="ref-badge">Ref: {{ $pago->referencia_transaccion }}</span>
            </div>
            ...
            <div class="barra-tiempo-wrap">
                ...
                <div class="barra-tiempo-label">
                    <span id="seg-restantes">{{ $timeout }}</span>s restantes
                </div>
            </div>
        ...
        <div id="pantalla-fallido" class="tarjeta" style="display: none">
            ...
            <a href="{{ route('cliente.pago', ['t' => $token, 'pedido_id' => $pedido->id]) }}" class="btn-reintentar">Intentar de nuevo</a>
        </div>
        ...
        <div id="pantalla-timeout" class="tarjeta" style="display: none">
            ...
            <a href="{{ route('cliente.pago', ['t' => $token, 'pedido_id' => $pedido->id]) }}" class="btn-reintentar">Volver a intentar</a>
        </div>
        ...
        <div id="pantalla-sin-conexion" class="tarjeta" style="display: none">
            ...
            <a href="{{ route('cliente.pago.pendiente', [$pago->id, 't' => $token]) }}" class="btn-reintentar">
                Recargar página
            </a>
        </div>

        {{-- ── DEV TOOLS (solo entornos locales) ── --}}
    ...
    <script>
        ...
        const TIMEOUT_SEG = {{ $timeout }};
        const RUTA_ESTADO    = "{!! route('cliente.pago.estado', [$pago->id, 't' => $token]) !!}";
        const RUTA_CONFIRMAR = "{!! route('cliente.confirmacion', ['t' => $token, 'pedido_id' => $pedido->id]) !!}";
        const RUTA_SIMULAR   = "{!! route('cliente.pago.simular', [$pago->id, 't' => $token]) !!}";
