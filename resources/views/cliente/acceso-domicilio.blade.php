...
<head>
    ...
    <title>Pedido a Domicilio – {{ $sucursal->nombre }}</title>
...
<body>
    ...
    <div class="contenedor">
        ...
        <div class="form-card">
            <form id="form-domicilio"
                  action="{{ route('cliente.domicilio.registro', ['empresa_slug' => $sucursal->empresa->slug, 'sucursal_slug' => $sucursal->slug]) }}"
                  method="POST">
                @csrf
                ...
                @else
                {{-- Si no hay barrios configurados, campo oculto vacío --}}
                <input type="hidden" name="barrio_id" value="">
                <div style="margin-bottom:1rem; padding:.75rem 1rem; background:rgba(193,127,62,.1); border:1px solid rgba(193,127,62,.25); border-radius:10px; font-size:.82rem; color:rgba(242,232,217,.7);">
                    Ingresa tu dirección y continúa para ver el menú.
                </div>
                @endif

                {{-- Tarjeta de envío dinámica (solo si hay barrios) --}}
                @if($barrios->isNotEmpty())
                <div id="card-envio">
                    ...
                    <div id="envio-info" style="display:none;">
                        ...
                        <div class="envio-row" id="row-sede-dir" style="display:none;">
                            <span class="envio-label">Dirección sede</span>
                            <span class="envio-sede" id="envio-sede-dir"></span>
                        </div>
                        <div class="envio-row" id="row-sede-tel" style="display:none;">
                            <span class="envio-label">Teléfono sede</span>
                            <span class="envio-sede" id="envio-sede-tel"></span>
                        </div>
                        <div class="envio-row">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        let debounceTimer;
        let tieneCobertura = true; // Si no hay selector de barrios, asumir cobertura
        ...
        if (selectBarrio) {
            selectBarrio.addEventListener('change', async () => {
                ...
                try {
                    const verifyUrl = `/{{ $sucursal->empresa->slug }}/{{ $sucursal->slug }}/domicilio/verificar-barrio?barrio_id=${barrioId}`;
                    ...
                    if (data.tiene_cobertura) {
                        ...
                        document.getElementById('envio-sede-nombre').textContent = data.sucursal_nombre;

                        const sedeDir = document.getElementById('envio-sede-dir');
                        const sedeTel = document.getElementById('envio-sede-tel');
                        const rowDir  = document.getElementById('row-sede-dir');
                        const rowTel  = document.getElementById('row-sede-tel');

                        if (rowDir) rowDir.style.display = 'none';
                        if (rowTel) rowTel.style.display = 'none';

                        if (sedeDir && data.sucursal_dir) {
                            sedeDir.textContent = data.sucursal_dir;
                            if (rowDir) rowDir.style.display = 'flex';
                        }
                        if (sedeTel && data.sucursal_tel) {
                            sedeTel.textContent = '📞 ' + data.sucursal_tel;
                            if (rowTel) rowTel.style.display = 'flex';
                        }
        ...
        // Si no hay barrios, el botón queda habilitado (no hay if-selectBarrio que lo deshabilite)
